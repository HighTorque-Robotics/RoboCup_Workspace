...
static dvision::ZedCamera *camera_;
...
int main(int argc, char *argv[]) {
  ros::init(argc, argv, "zed_ext_calib");
  ros::NodeHandle nh("~");
  ...
  // 创建 ZedCamera 对象，并初始化 ZED 相机
  camera_ = new dvision::ZedCamera();
...
void getPoints(ros::NodeHandle &nh) {
  ...
  for (unsigned int i = 0; i < p_tmp.size() / 2; i++) {
    ...
    ROS_DEBUG("points: %lf, %lf", p_tmp[i * 2], p_tmp[i * 2 + 1]);
  }
  point[0] = std::toupper(point[0]);
}
