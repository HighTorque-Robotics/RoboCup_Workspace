...
#include "dvision/gst_utility.hpp"
...
#include "dvision/cuda/cuda_mapped_memory.h"
#include "dvision/cuda/cuda_rgb.h"
#include "dvision/cuda/cuda_yuv.h"
...
const char* gstCameraSrcToString(gstCameraSrc src) {
  if (src == GST_SOURCE_NVCAMERA)
    return "GST_SOURCE_NVCAMERA";
  else if (src == GST_SOURCE_NVARGUS)
    return "GST_SOURCE_NVARGUS";
  else if (src == GST_SOURCE_V4L2)
    return "GST_SOURCE_V4L2";

  return "UNKNOWN";
}

// constructor
GstCameraImpl::GstCameraImpl() {
  mAppSink = NULL;
  mBus = NULL;
  mPipeline = NULL;
  mV4L2Device = -1;
...
// ConvertRGBA
bool GstCameraImpl::ConvertRGBA(void* input, void** output, bool zeroCopy) {
    ...
    for (uint32_t n = 0; n < NUM_RINGBUFFERS; n++) {
      if (zeroCopy) {
        ...
        if (!cudaAllocMapped(&cpuPtr, &gpuPtr, size)) {
          printf(LOG_CUDA
                 "gstCamera -- failed to allocate zeroCopy memory for %ux%xu "
                 "RGBA texture\n",
                 mWidth, mHeight);
          return false;
        }
        ...
        if (cpuPtr != gpuPtr) {
          printf(LOG_CUDA
                 "gstCamera -- zeroCopy memory has different pointers, please "
                 "use a UVA-compatible GPU\n");
          return false;
        }
        ...
        if (CUDA_FAILED(cudaMalloc(&mRGBA[n], size))) {
          printf(
              LOG_CUDA
              "gstCamera -- failed to allocate memory for %ux%u RGBA texture\n",
              mWidth, mHeight);
          return false;
        }
      }
  ...
  if (onboardCamera()) {
    ...
    if (CUDA_FAILED(cudaNV12ToRGBA32(
            (uint8_t*)input, (float4*)mRGBA[mLatestRGBA], mWidth, mHeight)))
      return false;
  } else {
    // USB webcam is RGB
    if (CUDA_FAILED(cudaRGB8ToRGBA32(
            (uchar3*)input, (float4*)mRGBA[mLatestRGBA], mWidth, mHeight)))
      return false;
  }
...
// onEOS
void GstCameraImpl::onEOS(_GstAppSink* sink, void* user_data) {
  printf(LOG_GSTREAMER "gstCamera onEOS\n");
}

// onPreroll
GstFlowReturn GstCameraImpl::onPreroll(_GstAppSink* sink, void* user_data) {
  printf(LOG_GSTREAMER "gstCamera onPreroll\n");
  return GST_FLOW_OK;
}

// onBuffer
GstFlowReturn GstCameraImpl::onBuffer(_GstAppSink* sink, void* user_data) {
  // printf(LOG_GSTREAMER "gstCamera onBuffer\n");

  if (!user_data) return GST_FLOW_OK;

  GstCameraImpl* dec = (GstCameraImpl*)user_data;
...
// Capture
bool GstCameraImpl::Capture(void** cpu, void** cuda, unsigned long timeout) {
  mWaitMutex->lock();
  ...
  if (cpu != NULL) *cpu = mRingbufferCPU[latest];

  if (cuda != NULL) *cuda = mRingbufferGPU[latest];
...
// checkBuffer
void GstCameraImpl::checkBuffer() {
  if (!mAppSink) return;
  ...
  if (!gstSample) {
    printf(LOG_GSTREAMER
           "gstreamer camera -- gst_app_sink_pull_sample() returned NULL...\n");
    return;
  }
  ...
  if (!gstBuffer) {
    printf(LOG_GSTREAMER
           "gstreamer camera -- gst_sample_get_buffer() returned NULL...\n");
    return;
  }
  ...
  if (!gstData) {
    printf(LOG_GSTREAMER
           "gstreamer camera -- gst_buffer had NULL data pointer...\n");
    release_return;
  }
  ...
  if (!gstCapsStruct) {
    printf(LOG_GSTREAMER
           "gstreamer camera -- gst_caps had NULL structure...\n");
    release_return;
  }
  ...
  if (!gst_structure_get_int(gstCapsStruct, "width", &width) ||
    ...
    printf(LOG_GSTREAMER
           "gstreamer camera -- gst_caps missing width/height...\n");
    release_return;
  }

  if (width < 1 || height < 1) release_return;
  ...
  // printf(LOG_GSTREAMER "gstreamer camera recieved %ix%i frame (%u bytes, %u
  ...
  if (!mRingbufferCPU[0]) {
    for (uint32_t n = 0; n < NUM_RINGBUFFERS; n++) {
      if (!cudaAllocMapped(&mRingbufferCPU[n], &mRingbufferGPU[n], gstSize))
        printf(
            LOG_CUDA
            "gstreamer camera -- failed to allocate ringbuffer %u  (size=%u)\n",
            n, gstSize);
    }

    printf(LOG_CUDA
           "gstreamer camera -- allocated %u ringbuffers, %u bytes each\n",
  ...
  // printf(LOG_GSTREAMER "gstreamer camera -- using ringbuffer #%u for next
  // frame\n", nextRingbuffer);
...
// buildLaunchStr
bool GstCameraImpl::buildLaunchStr(gstCameraSrc src) {
  ...
  // #define CAPS_STR "video/x-raw(memory:NVMM), width=(int)2592,
  // height=(int)1944, format=(string)I420, framerate=(fraction)30/1" #define
  // CAPS_STR "video/x-raw(memory:NVMM), width=(int)1920, height=(int)1080,
  ...
  if (onboardCamera() && src != GST_SOURCE_V4L2) {
    ...
    NV_TENSORRT_MAJOR < 5      // if JetPack 3.1-3.3 (different flip-method)
    ...
    if (src == GST_SOURCE_NVCAMERA)
      ss << "nvcamerasrc fpsRange=\"30.0 30.0\" ! video/x-raw(memory:NVMM), "
            "width=(int)"
         << mWidth << ", height=(int)" << mHeight
      ...
      ss << "nvarguscamerasrc ! video/x-raw(memory:NVMM), width=(int)" << mWidth
         << ", height=(int)" << mHeight
         << ", framerate=30/1, format=(string)NV12 ! nvvidconv flip-method="
         << flipMethod << " ! ";
    ...
    ss << "format=YUY2 ! videoconvert ! video/x-raw, format=BGR ! videoconvert "
          "!";
#else
    ss << "format=BGR ! videoconvert ! video/x-raw, format=BGR ! videoconvert "
          "!";
#endif
...
// Create
GstCameraImpl* GstCameraImpl::Create(uint32_t width, uint32_t height,
                                     int v4l2_device) {
  if (!gstreamerInit()) {
  ...
  GstCameraImpl* cam = new GstCameraImpl();
  ...
  if (!cam->init(GST_SOURCE_NVCAMERA)) {
    ...
    if (!cam->init(GST_SOURCE_NVARGUS)) {
      ...
      if (cam->mV4L2Device < 0) cam->mV4L2Device = 0;
...
// Create
GstCameraImpl* GstCameraImpl::Create(int v4l2_device) {
  return Create(DefaultWidth, DefaultHeight, v4l2_device);
}

// init
bool GstCameraImpl::init(gstCameraSrc src) {
  GError* err = NULL;
  ...
  if (!appsinkElement || !appsink) {
    printf(LOG_GSTREAMER
           "gstreamer failed to retrieve AppSink element from pipeline\n");
    return false;
  }
...
// Open
bool GstCameraImpl::Open() {
  // transition pipline to STATE_PLAYING
  printf(LOG_GSTREAMER
         "gstreamer transitioning pipeline to GST_STATE_PLAYING\n");
  ...
  if (result == GST_STATE_CHANGE_ASYNC) {
    ...
    printf(LOG_GSTREAMER
           "gstreamer failed to set pipeline state to PLAYING (error %u)\n",
           result);
    return false;
  }
...
// checkMsgBus
void GstCameraImpl::checkMsgBus() {
  while (true) {
