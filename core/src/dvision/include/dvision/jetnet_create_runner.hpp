...
class YoloRunnerFactory {
 ...
 public:
  ...
  typedef std::shared_ptr<jetnet::ModelRunner<jetnet::CvLetterBoxPreProcessor,
                                              jetnet::YoloPostProcessor>>
      RunnerType;

  YoloRunnerFactory(size_t num_classes, float threshold, float nms_threshold,
                    size_t batch_size, std::vector<float> anchor_priors,
  ...
  std::tuple<PreType, RunnerType, PostType> create_yolov2() {
    ...
    auto logger =
        std::make_shared<jetnet::Logger>(nvinfer1::ILogger::Severity::kINFO);
    ...
    if (!m_anchor_priors.empty()) {
      ...
      m_anchor_priors =
          std::vector<float>{0.57273, 0.677385, 1.87446, 2.06253, 3.33843,
                             5.47434, 7.88282,  3.52778, 9.77052, 9.16828};
    }

    std::vector<unsigned int> channel_map{
        0, 1, 2};  // read_image read RGB order, network expects RGB order
    auto pre = std::make_shared<jetnet::CvLetterBoxPreProcessor>(
        input_blob_name, channel_map, logger);

    std::vector<jetnet::YoloPostProcessor::OutputSpec> output_specs = {
        jetnet::YoloPostProcessor::OutputSpec{output_blob_name, m_anchor_priors,
                                              m_num_classes}};

    auto post = std::make_shared<jetnet::YoloPostProcessor>(
        input_blob_name, jetnet::YoloPostProcessor::Type::Yolov2, output_specs,
        m_threshold, logger, [=](std::vector<jetnet::Detection> &detections) {
          jetnet::nms_sort(detections, m_nms_threshold);
        });

    auto runner =
        std::make_shared<jetnet::ModelRunner<jetnet::CvLetterBoxPreProcessor,
                                             jetnet::YoloPostProcessor>>(
            plugin_fact, pre, post, logger, m_batch_size, m_enable_profiling);
  ...
  std::tuple<PreType, RunnerType, PostType> create_yolov3() {
    ...
    if (!m_anchor_priors.empty()) {
      std::cout << "Using custom anchor priors" << std::endl;
      anchor_priors3 = std::vector<float>(m_anchor_priors.begin(),
                                          m_anchor_priors.begin() + 6);
      anchor_priors2 = std::vector<float>(m_anchor_priors.begin() + 6,
                                          m_anchor_priors.begin() + 12);
      anchor_priors1 = std::vector<float>(m_anchor_priors.begin() + 12,
                                          m_anchor_priors.end());
    } else {
      std::cout << "Using default anchor priors" << std::endl;
    ...
    std::vector<jetnet::YoloPostProcessor::OutputSpec> output_specs = {
        jetnet::YoloPostProcessor::OutputSpec{output_blob1_name, anchor_priors1,
                                              m_num_classes},
        jetnet::YoloPostProcessor::OutputSpec{output_blob2_name, anchor_priors2,
                                              m_num_classes},
        jetnet::YoloPostProcessor::OutputSpec{output_blob3_name, anchor_priors3,
                                              m_num_classes}};

    auto post = std::make_shared<jetnet::YoloPostProcessor>(
        input_blob_name, jetnet::YoloPostProcessor::Type::Yolov3, output_specs,
        m_threshold, logger, [=](std::vector<jetnet::Detection> &detections) {
          jetnet::nms_sort(detections, m_nms_threshold);
        });
  ...
  std::tuple<PreType, RunnerType, PostType> create_yolov3_tiny() {
    ...
    if (!m_anchor_priors.empty()) {
      std::cout << "Using custom anchor priors" << std::endl;
      anchor_priors2 = std::vector<float>(m_anchor_priors.begin(),
                                          m_anchor_priors.begin() + 6);
      anchor_priors1 = std::vector<float>(m_anchor_priors.begin() + 6,
                                          m_anchor_priors.end());
    } else {
      std::cout << "Using default anchor priors" << std::endl;
