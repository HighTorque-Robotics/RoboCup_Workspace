...
/*! Used for capturing frames */
  ...
  /*! Used for implement concurrent work */
  dprocess::DConcurrent concurrent_;
  //! Worksapce for storaging processed vision information
  VisionSharedInfo* workspace_;

  //! Instance of Frame class
  /*! Used for storaging camera frames in various color spaces and encoding
 ...
 private:
  ...
  Tracker tracker_;
  //! Another instance of Projection class in order to run in seperate thread
  ...
  // Detectors
  //! Detectors for the ball
  BallDetector ball_;
  //! Detectors for the center circle
  CircleDetector circle_;
  //! Detectors for the corner
  CornerDetector corner_;
  //! Classifier for the white lines to determine it's horizonal or vertical
  LineClassifier line_classifier_;
  //! Detectors for the goal
  GoalDetector goal_;
  ...
  //! Delta yaw value between current one and previou one
  double imu_yaw_delta_ = 0;
  //! Corrected yaw value of robot
  double vision_yaw_ = M_PI / 2;  // 机器人yaw角度
  ...
  //! Delta x value between current one and previou one in world coordinate
  double vision_x_delta_ = 0;  // 两帧间的世界坐标系位置差 delta_x
  //! Delta y value between current one and previou one in world coordinate
