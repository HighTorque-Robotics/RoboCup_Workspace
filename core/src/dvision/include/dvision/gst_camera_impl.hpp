...
const char* gstCameraSrcToString(
    gstCameraSrc src); /**< Text version of gstCameraSrc enum */
...
class GstCameraImpl {
 public:
  // Create camera
  static GstCameraImpl* Create(
      int v4l2_device = -1);  // use onboard camera by default (>=0 for V4L2)
  static GstCameraImpl* Create(uint32_t width, uint32_t height,
                               int v4l2_device = -1);
  ...
  // Takes in captured YUV-NV12 CUDA image, converts to float4 RGBA (with pixel
  // intensity 0-255) Set zeroCopy to true if you need to access ConvertRGBA
  // from CPU, otherwise it will be CUDA only.
